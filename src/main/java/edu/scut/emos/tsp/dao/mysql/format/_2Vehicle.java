...
public class _2Vehicle {
	...
	public static Vehicle a(DVehicle dVehicle, List<DRoute> dRoutes) {
		...
		while(loadeddRoutesmapiterator.hasNext()) {
			...
			loadedTasks.add(new ScheduleTask(dOrdermap.get(entry.getValue().getRouteorderid()), entry.getValue().getAction().intValue()));
		}
		...
		while(plandRoutesmapiterator.hasNext()) {
			...
			planTasks.add(new ScheduleTask(dOrdermap.get(entry.getValue().getRouteorderid()), entry.getValue().getAction().intValue()));
		}
		...
		// 创建Vehicle对象
		return new Vehicle(dVehicle.getVehicleid(), dVehicle.getLength(), dVehicle.getWidth(), dVehicle.getHeight(), dVehicle.getTypeofvehicle(), dVehicle.getMaxweight(), dVehicle.getMaxvolume(), new Position(dVehicle.getLatestlatitude(), dVehicle.getLatestlongitude()), dVehicle.getGpsuploadtime(), dVehicle.getFuelconsumption(), dVehicle.getLastupdatetime(), dVehicle.getLoadedweight(), dVehicle.getLoadedvolume(), ((dVehicle.getIslocked().intValue() == 1) ? true : false), route);
	}
}
